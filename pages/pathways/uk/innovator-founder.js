...
function MvpModal({ onClose }) {
  ...
  return (
    <div className={ev.modalOverlay} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className={ev.modalCard} role="dialog" aria-modal="true" aria-labelledby="mvp-modal-title">
        ...
        {done ? (
          <div className={ev.modalSuccess}>
            ...
            <div className={ev.modalSuccessText}>
              Your details have been saved. Click below to start your free MVP validation — it takes around 10 minutes.
            </div>
            <a href={MVP_URL} target="_blank" rel="noopener noreferrer" className={ev.modalSuccessBtn}>
              🧪 Start MVP Validator →
            </a>
          </div>
        ) : (
          <>
            ...
            <div id="mvp-modal-title" className={ev.modalTitle}>Before you start your validation</div>
            <div className={ev.modalSub}>
              Share a few quick details so we can follow up with personalised guidance on your
              Innovator Founder Visa journey.
            </div>
            <form className={ev.modalForm} onSubmit={handleSubmit} noValidate>
              <div className={ev.modalField}>
                <label className={ev.modalLabel} htmlFor="mvp-name">Your Full Name</label>
                <input
                  id="mvp-name"
              ...
              <div className={ev.modalField}>
                <label className={ev.modalLabel} htmlFor="mvp-email">Email Address</label>
                <input
                  id="mvp-email"
                  className={ev.modalInput}
                  type="email"
              ...
              <div className={ev.modalField}>
                <label className={ev.modalLabel} htmlFor="mvp-phone">
                  Phone Number{' '}
                  <span style={{ fontWeight: 400, color: 'var(--muted)', fontSize: '0.72rem' }}>(with country code)</span>
                </label>
                <input
                  id="mvp-phone"
                  className={ev.modalInput}
              ...
              <div className={ev.modalField}>
                <label className={ev.modalLabel} htmlFor="mvp-biz">
                  Business Name
              ...
              <button type="submit" className={ev.modalSubmit} disabled={submitting}>
                {submitting ? 'Saving…' : 'Continue to MVP Validator →'}
              </button>
            </form>
...
export default function InnovatorFounder() {
  ...
  return (
    <>
    {mvpOpen && <MvpModal onClose={() => setMvpOpen(false)} />}
    <PathwayPage
      meta={{
